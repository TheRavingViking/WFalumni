...
box-shadow: 0px 10px 30px -1px rgba(0,0,0,0.20);">
        ...
        <table class="table">
            <thead><h1>Bevoegdheid veranderen:</h1></thead>
            <tr>
                <td>ID</td>
                <td>Voornaam</td>
                <td>Achternaam</td>
            ...
            @foreach($users as $user)
                <tr>
                    <form action="{{ route('admin.assign') }}" method="POST">
                        ...
                        <td>{{ $user->email }}
                        <td><input type="radio" {{ $user->bevoegdheid === 1 ? 'checked' : '' }} name="bevoegdheid"
                                   value="1"></td>
                        <td><input type="radio" {{ $user->bevoegdheid === 2 ? 'checked' : '' }} name="bevoegdheid"
                                   value="2"></td>
                        <td><input type="radio" {{ $user->bevoegdheid === 3 ? 'checked' : '' }} name="bevoegdheid"
                                   value="3"></td>
                        {{ csrf_field() }}
                        <td>
